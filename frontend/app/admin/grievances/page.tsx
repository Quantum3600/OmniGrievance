                  ...
                  <Card key={g.ticket_id} className="border-slate-200 shadow-sm flex flex-col sm:flex-row overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div className="flex-1 p-6">
                      <div className="flex justify-between items-start mb-3">
                        <Badge variant="outline" className={`font-black text-[9px] tracking-widest border-2 ${g.status === "ASSIGNED" ? "border-amber-200 text-amber-600 bg-amber-50" :
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {activeByTeam[teamKey].tasks.map((task: any) => (
                  <Card key={task.ticket_id} className="border-slate-200 shadow-sm relative overflow-hidden group">
                    <div className={`absolute top-0 left-0 w-1.5 h-full ${task.status === "ASSIGNED" ? "bg-amber-400" :
                ...
                {employees
                  ...
                  .map(emp => (
                    <div
                      ...
                      className={`flex items-start gap-4 p-4 rounded-2xl border-2 transition-all ${emp.is_busy
                    ...
                    >
                      <div className={`mt-0.5 w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${emp.is_busy ? 'bg-slate-200 border-slate-200' : selectedEmployees.includes(emp.employee_id_tag) ? 'bg-blue-600 border-blue-600' : 'border-slate-300'
                        }`}>
